...
class Program
{
    ...
    public static void Main(string[] args)
    {
        ...
        Font font = new Font(
            "C:/Code/RaycastingEngine/RayCastingEngineProject/Raycaster/bin/Debug/net6.0/resources/arial.ttf");
        Text text = new Text("", font);
        text.Position = new Vector2f(SCREEN_WIDTH - 200, 20);
        ContextSettings context = new ContextSettings() { AntialiasingLevel = 16 };
        RenderWindow window = new RenderWindow(new VideoMode(SCREEN_WIDTH, SCREEN_HEIGHT), "Raycaster", Styles.Default,
        ...
        window.MouseButtonPressed += (sender, args) => MouseButtonPressed(sender, args, player, sounds);
        ...
        while (window.IsOpen)
        {
            ...
            player.DrawShotgun(window); // Рисуем шотган
    ...
    private static void MouseButtonPressed(object sender, MouseButtonEventArgs args, Player player, Sounds sounds)
    {
        if (args.Button == Mouse.Button.Left)
        {
            player.StartFiring();
            sounds.playSound(sounds.shotgunSound);
        }
    }
